...
As a long-time infrastructure enthusiast, I've had my share of dalliances with various tools and workflows. For a good while, <ExternalLink href="https://www.runatlantis.io/">Atlantis</ExternalLink> was my reliable partner in managing <ExternalLink href="https://www.hashicorp.com/en/products/terraform">Terraform</ExternalLink> deployments. It brought order to the chaos of collaborative infrastructure-as-code, and for that, I'll always be grateful.
...
So, how <ExternalLink href="/">BigConfig</ExternalLink> is solving these limits of Atlantis?
...
* **Better developer experience:** Let's be honest, `terraform` is fundamentally an interactive tool. The scenario where a developer can write a non-trivial change in one go and then run it in Atlantis without bugs is rare. Most of the time we develop the terraform code incrementally with a sequence of `plan` and `apply` and the PR process increases the delivery time.

### The Verdict: Atlantis is not compatible with modern software development

Let me be clear: Atlantis isn't a "bad" tool. For many teams, especially those that are ok with the limits of the GitOps approach, it can still be a fantastic entry point. But for me, modern software development principles should be applied to both development and operations.
