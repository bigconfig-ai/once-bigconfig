...
`big-config` is a workflow and a template engine that enables you to have a
zero-cost `build` step before running any `devops` tool like `Makefile`, `terraform`, `k8s`,
and `ansible`.

## Zero-cost build step
This example is for OpenTofu but `big-config` can be used with any `devops` tool.
<Steps>
1. Inside a `big-config` project, create an `alias`.
   ``` shell
